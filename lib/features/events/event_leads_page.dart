...
class _EventLeadsPageState extends State<EventLeadsPage> {
  ...
  Future<void> _createLead() async {
    ...
    if (_name.text.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Please enter lead name')));
      return;
    }
    ...
    try {
      ...
      await _dbService.createEventLead({
        ...
        'interested_products':
            _products.text.trim().isNotEmpty
                            ...
                            children: [
                              ...
                              DropdownButtonFormField<String>(
                                ...
                                items: const [
                                  ...
                                  DropdownMenuItem(
                                    value: '1_quarter',
                                    child: Text('Within 1 Quarter'),
                                  ),
                                  DropdownMenuItem(
                                    value: 'future',
                                    child: Text('Future Consideration'),
                                  ),
                                ],
  ...
  Widget _buildSummaryCard() {
    ...
    final withPhone =
        _leads
            .where(
              (l) => l['phone'] != null && l['phone'].toString().isNotEmpty,
            )
            .length;
    final withEmail =
        _leads
            .where(
              (l) => l['email'] != null && l['email'].toString().isNotEmpty,
            )
            .length;
